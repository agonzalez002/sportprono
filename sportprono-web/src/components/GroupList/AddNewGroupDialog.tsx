...
interface AddNewGroupType {
  ...
  onGroupCreated: (newGroup: GroupType) => void;
}

function AddNewGroupDialog({ open, setOpen, onGroupCreated }: AddNewGroupType) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await createGroup(uploadData, authData.token);
      if (response && response.group) {
        setName("");
        setImageUrl(emptyFile);
        setAccess("private");
        setGroupLogo(undefined);
        toast.success(response.message || "Nouveau Groupe créé !");
  ...
  const private_text =
    "Seul les personnes possédant le code d'accés pourront rejoindre votre groupe.";
